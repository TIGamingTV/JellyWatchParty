...
// utils.log (utils/log.js); it tolerates a missing OWP.ui (guarded with
// `OWP.ui &&` everywhere it's used).
require('../utils/video.js');
require('../utils/log.js');
require('../playback/sync.js');

const { DRIFT_CORRECTION_ENTER_SEC, DRIFT_CORRECTION_EXIT_SEC, DRIFT_SOFT_MAX_SEC, PLAYBACK_RATE_MIN, PLAYBACK_RATE_MAX } = OWP.constants;
...
describe('playback/sync syncLoop drift correction', () => {
  ...
  beforeEach(() => {
    ...
    OWP.state.isDriftCorrecting = false;
    OWP.state.isInitialSync = false;
    OWP.state.syncCooldownUntil = 0;
    OWP.state.syncStatus = 'unknown';
    ...
    OWP.state.lastSyncServerTs = OWP.utils.getServerNow();
    OWP.state.lastSyncPosition = 10.0;
  });
  ...
  it('stays quiet (1x) when drift is under the enter threshold', () => {
    ...
    assert.equal(OWP.state.isDriftCorrecting, false);
  ...
  it('hysteresis: keeps correcting between the exit and enter thresholds once started', () => {
    ...
    OWP.playback.syncLoop();
    assert.equal(OWP.state.isDriftCorrecting, true, 'should still be correcting inside the hysteresis band');
