...
- `JellyWatchPartyPlugin.csproj` — package metadata; targets `net10.0`
  (Jellyfin 12.x only — Jellyfin 10.11.x support was dropped, see
  `Directory.Build.props`), with the `Jellyfin.Controller`/`Jellyfin.Model`
  version pinned in `Directory.Build.props`.
